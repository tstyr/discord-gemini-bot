...
4. フォームに入力:
   - **APP NAME**: `discord bot`（任意の名前）
   - **ICON URL**: `https://example.com/icon.png`（適当なURLでOK）
   - **APP WEBSITE URL**: `https://example.com`（適当なURLでOK）
   - **REDIRECT URI**: `https://example.com/callback`（適当なURLでOK）
5. 「Save」をクリック
...
- `#lyrics-stream` チャンネルで状態メッセージを確認
